...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 构建标记：改这个文件时顺手改一下。页面控制台里 `__DSH_TOOLBOX__.build` 能读到它，
     * 用来一眼确认"存盘后的热重载到底有没有到位"（热重载没到位时它还是旧值）。
     */
    const BUILD = 'toolbox-3 (plate→toolbar recipe, click-outside dismiss)'
    ...
    /**
     * 已知工具的展示信息，按 DOM 指纹匹配（CSS 模块的类名带哈希前缀，用子串匹配）。
     * 匹配不到就退回 DOM 上能读到的名字——这样以后新装的插件也能自动进菜单。
     */
    const KNOWN = [
      {
    ...
    const CSS = [
      ...
      // ── 第三方底板修正：浏览器插件（dsh-browser-scope）底板对齐工具栏 ────────────────
      ...
      // 主题没装时退回 18px/160%)，于是透明度和质感与工具栏一致。
      ...
      return React.createElement(
        ...
        open
          ? React.createElement(
            ...
            React.createElement('div', { className: 'tbx-list' }, rows),
            notice === '' ? null : React.createElement('div', { className: 'tbx-notice' }, notice),
          )
          : null,
      )
    }
